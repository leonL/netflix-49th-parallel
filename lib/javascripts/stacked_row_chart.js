...
FLIX.StackedRowChart.prototype = {
  ...
  renderXVals: function() {
    ...
    textTemplate = _.template(
      "<tspan class='sub-totals'>"+
        "<tspan class='exclusive'><%= exclusiveSubTotal %></tspan> / "+
          "<tspan class='shared'><%= sharedSubTotal %></tspan>"+
      "</tspan>"+
      "<tspan class='percentages'>"+
        "<tspan class='exclusive'><%= exclusivePercentages %></tspan> / "+
          "<tspan class='shared'><%= sharedPercentages %></tspan>"+
      "</tspan>"+
      "<tspan class='total'><%= total %></tspan>"
    ),
    that = this;
    ...
    sharedBarsD3.append('text')
      ...
      .attr("text-anchor", 'start')
      .html(function(d) {
        var totalVal = d.y0 + d.y,
        text;
        if(totalVal === 0) {
          text = "";
        } else {
          var exPercent = String(Math.round(d.y0 / totalVal * 100)) + "%",
              shrdPercent = String(Math.round(d.y / totalVal * 100)) + "%";
          text = textTemplate(
            {
              exclusiveSubTotal: d.y0, sharedSubTotal: d.y,
              exclusivePercentages: exPercent, sharedPercentages: shrdPercent,
              total: totalVal
            });
        }
        return text;
